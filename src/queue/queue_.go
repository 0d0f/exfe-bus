...
func NewQueue_(config *model.Config, redis broker.Redis) (*Queue_, error) {
...
func (q *Queue_) SetRoute(r gobus.RouteCreater) error {
	json := new(gobus.JSON)
...
func (q *Queue_) Push(param map[string]string, arg Push) (int, error) {
...
func (q *Queue_) instant(service, groupKey string, recipients []model.Recipient, data interface{}) (int, error) {
	ret := len(recipients)
...
func (q *Queue_) delay(repo delayrepo.Repo, service, groupKey string, recipients []model.Recipient, data interface{}) (int, error) {
	ret := 0
	d, ok := data.(map[string]interface{})
...
func (q *Queue_) callback(name string) func(string, [][]byte) {
