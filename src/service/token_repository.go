...
const (
	STORE          = "INSERT INTO `tokens` (`key`, `hash`, `user_id`, `scope`, `client`, `data`, `expires_at`, `created_at`, `touched_at`) VALUES (?, ?, ?, ?, ?, ?, ?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP())"
	FIND_BY_KEY    = "SELECT `key`, hash, user_id, scope, client, data, touched_at, expires_at FROM `tokens` WHERE expires_at>UNIX_TIMESTAMP() AND `key`=?"
	FIND_BY_HASH   = "SELECT `key`, hash, user_id, scope, client, data, touched_at, expires_at FROM `tokens` WHERE expires_at>UNIX_TIMESTAMP() AND `hash`=?"
...
func (r *TokenRepo) Store(token token.Token) error {
	_, err := r.db.Exec(STORE, token.Key, token.Hash, token.UserId, token.Scope, token.Client, token.Data, token.ExpiresAt)
	if err != nil {
		return err
	}
	r.db.Exec(DELETE)
...
func (r *TokenRepo) FindByKey(key string) ([]token.Token, error) {
	...
	for rows.Next() {
		err := rows.Scan(&token.Key, &token.Hash, &token.UserId, &token.Scope, &token.Client, &token.Data, &token.TouchedAt, &token.ExpiresAt)
		if err != nil {
			return ret, err
		}
