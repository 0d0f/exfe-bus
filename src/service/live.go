...
func (h LiveService) Card_(data here.Data) []string {
	...
	h.config.Log.Info("|live|add|t|%s|card|%s|name|%s|long|%s|lang|%s|trait|%s", data.Token, data.Card.Id, data.Card.Name, data.Longitude, data.Latitude, data.Traits)
	err := h.here.Add(&data)
	...
	go func() {
		...
		for {
			...
			for token := range group.Data {
				...
				if group.Name == "" {
					for _, c := range cards {
						config.Log.Info("|live|clear|t|%s|card|%s|name|%s|long|%s|lang|%s|trait|%s", token, c.Id, c.Name, group.CenterLongitude, group.CenterLatitude, group.Traits)
					}
