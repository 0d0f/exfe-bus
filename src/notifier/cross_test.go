...
func TestCrossInvitationTwitter(t *testing.T) {
	...
	assert.Equal(t, err, nil)
	t.Logf("private:---------start---------\n%s\n---------end----------", private)
	expectPrivate := "\n\n\n\nSuccessfully gathering \\(\"Test Cross\"\\), \\(4:45PM on Tue, Oct 23 2012\\). 6 invited: email1 name, email2 name, twitter3 name... http://site/url/#!token=recipient_twitter1_token"
	assert.Equal(t, private, expectPrivate)
